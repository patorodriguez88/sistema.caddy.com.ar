...
$sql=mysql_query("SELECT nombrecliente,id FROM Clientes WHERE NdeCliente='$_SESSION[NCliente]'");
$NombreCliente=mysql_fetch_array($sql);
$idCliente=$NombreCliente[id];
...
echo "<a id='cerrar' href='#'>&times;</a>";
echo "<div class='popupContent' style='margin-top:0px'>";

//VERIFICO SI MI PEDIDO ES EL PROXIMO
// PRIMERO BUSCO EL RECORRIDO
$sql=mysql_query("SELECT Recorrido FROM TransClientes WHERE CodigoSeguimiento='$_GET[cod]' AND Eliminado='0'");
if($datorec=mysql_fetch_array($sql)){

  debug_to_console("Recorrido ".$datorec[Recorrido]);

  $sqllog=mysql_query("SELECT * FROM HojaDeRuta WHERE Estado='Abierto' AND Recorrido='$datorec[Recorrido]' AND Eliminado='0' ORDER BY Posicion ASC LIMIT 0,1");   
  ...
  if($datosqllog[Seguimiento]==$_GET[cod]){
    ...
    if($datovehi[Patente]<>''){
      ...
      include('mapaseguimiento.html');
      }else{
    echo "<img  src='images/proximo.png'>";  
      }
    }else{
    echo "<img  src='images/proximo.png'>";
    }
...
//  DESDE ACA MUESTRO LA TABLA     
  ...
  $sqlRelaciones=mysql_query("SELECT id FROM Clientes WHERE Relacion='$idCliente' AND AdminEnvios='1'");
  $cuantos=mysql_num_rows($sqlRelaciones);
  $inicio=1;

  while($rowr = mysql_fetch_array($sqlRelaciones)){
  if($inicio==$cuantos){
    $coma='';  
    }else{
    $coma=',';  
  ...
  if($cuantos>1){
  $idfinal=$idCliente.",".$id;
  }else{
 $idfinal="'".$idCliente."'";
}
  
  $sqlCtaCte=mysql_query("SELECT * FROM TransClientes WHERE IngBrutosOrigen IN($idfinal) AND Entregado = '0' AND Eliminado='0' AND Haber='0' ORDER BY Fecha");
    ...
    echo "<th style='background-color:#E24F30;'>Guia</th>";
...
// $Cantidad+=$row[Cantidad];      
  ...
  if(($Estado=='En Transito')||($Estado=='EnOrigen')||($Estado=='Pendiente..')||($Estado=='A Retirar')||($Estado=='En Origen')){
    ...
    echo "<td style='padding:5px'>$row[RazonSocial]</td>";
...
//       echo "<td style='padding:5px'>$row[CodigoSeguimiento]</td>";
    ...
    echo "<td align='center'><a target='_blank' href='http://www.caddy.com.ar/SistemaTriangular/Ventas/Informes/Remitopdf2.php?CS=".$row[CodigoSeguimiento]."'><input type='image' src='images/Remito.png' width='30' height='30' border='0' style='float:center;'></td>";
