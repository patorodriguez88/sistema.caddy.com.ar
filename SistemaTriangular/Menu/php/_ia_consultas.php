...
function detectarClienteConsulta($mysqli, $q)
{
    ...
    $like = '%' . $mysqli->real_escape_string($busqueda) . '%';
...
/* =========================
   ...
   Ej: "Dame las ventas de Ferniplast del mes pasado"
========================= */
...
if (
    ...
    $stmt = $mysqli->prepare("
        SELECT 
            ...
            SUM(CASE 
                WHEN Retirado = 0 THEN 1 
                ELSE 0 
            END) AS cantidad_envios,

            SUM(CASE 
                WHEN Retirado = 1 THEN 1 
                ELSE 0 
            END) AS cantidad_recepciones,

            SUM(CASE 
                WHEN Entregado = 1 THEN 1 
                ELSE 0 
            END) AS cantidad_entregados,

            SUM(CASE 
                WHEN Devuelto = 1 THEN 1 
                ELSE 0 
            END) AS cantidad_devueltos,
    ...
    if (!$stmt) {
        salir([
            'success' => 0,
            'msg' => 'Error preparando consulta de ventas por cliente.'
        ]);
    }

    $stmt->bind_param("sss", $razonSocial, $fechaDesdeSQL, $fechaHastaSQL);
    $stmt->execute();
    $res = $stmt->get_result();
    $datos = $res->fetch_assoc();
    $stmt->close();

    $total = (int)$datos['cantidad_total'];
    $envios = (int)$datos['cantidad_envios'];
    $recepciones = (int)$datos['cantidad_recepciones'];
    $entregados = (int)$datos['cantidad_entregados'];
    $devueltos = (int)$datos['cantidad_devueltos'];
    $pendientes = (int)$datos['cantidad_pendientes'];

    $detalle = "
        🚚 <strong>Envíos:</strong> $envios<br>
        📦 <strong>Recepciones / retiros:</strong> $recepciones<br>
        ✅ <strong>Entregados:</strong> $entregados<br>
        ⏳ <strong>Pendientes:</strong> $pendientes<br>
        ↩️ <strong>Devueltos:</strong> $devueltos<br>
        📋 <strong>Total movimientos:</strong> $total<br>
    ";
    ...
    $detalle .= "
        <hr class='my-1'>
        <small>
            Criterio: TransClientes.RazonSocial = '{$razonSocial}', Fecha entre {$fechaDesdeSQL} y {$fechaHastaSQL}.
        </small>
    ";
...
/* =========================
   LOCALIDADES / CÓDIGOS POSTALES / COBERTURA
   Ej:
   "¿A qué localidades llegamos?"
   "¿De qué localidad es el código postal 5186?"
   "¿Cuál es el código postal de Alta Gracia?"
========================= */

if (
    contieneAlguna($q, ['localidad', 'localidades', 'codigo postal', 'cp', 'cobertura', 'llegamos', 'llegan'])
) {

    /* CP -> LOCALIDAD */
    if (preg_match('/\b(\d{4,5})\b/', $q, $m)) {
        $cp = $mysqli->real_escape_string($m[1]);
        ...
        if ($res->num_rows === 0) {
            ...
            salir([
                'success' => 0,
                'msg' => "No encontré localidades con el código postal <strong>$cp</strong>."
            ]);
        }
        ...
        while ($row = $res->fetch_assoc()) {
            $detalle .= "
                ...
                <small>
                    CP: {$row['Cp']} | Recorrido: {$row['Recorrido']} | Km: {$row['Km']} | Día salida: {$row['DiaSalida']}
                </small><br>
                <small>Web: " . ((int)$row['Web'] === 1 ? 'Sí' : 'No') . "</small>
                <hr class='my-1'>
            ";
            $i++;
        }
        ...
        salir([
            'success' => 1,
            'respuesta' => "Encontré localidades para el código postal <strong>$cp</strong>.",
            'detalle' => $detalle
        ]);
    }
    ...
    /* LOCALIDAD -> CP */
    if (
        contieneAlguna($q, ['codigo postal', 'cp'])
        && !preg_match('/\b(\d{4,5})\b/', $q)
    ) {
        $busqueda = $q;
        ...
        if (strlen($busqueda) < 3) {
            salir([
                'success' => 0,
                'msg' => 'Decime la localidad. Ej: “Me decís el código postal de Alta Gracia”.'
            ]);
        }
        ...
        if ($res->num_rows === 0) {
            ...
            salir([
                'success' => 0,
                'msg' => "No encontré localidades parecidas a <strong>$busqueda</strong>."
            ]);
        }
        ...
        while ($row = $res->fetch_assoc()) {
            $detalle .= "
                ...
                <small>
                    Recorrido: {$row['Recorrido']} | Km: {$row['Km']} | Día salida: {$row['DiaSalida']} | Web: " . ((int)$row['Web'] === 1 ? 'Sí' : 'No') . "
                </small>
                <hr class='my-1'>
            ";
            $i++;
    ...
    /* LISTAR LOCALIDADES DONDE LLEGAMOS */
    if (
        contieneAlguna($q, ['localidades', 'cobertura', 'llegamos', 'llegan', 'zonas'])
    ) {
        ...
        salir([
            'success' => 1,
            'respuesta' => $soloWeb
                ? "Estas son las localidades habilitadas en la web."
                : "Estas son algunas localidades a las que llegamos.",
...
if (
    ...
    if ($busqueda === '') {
        salir([
            'success' => 0,
            'msg' => 'Decime qué tarifa querés consultar. Ej: “Cuánto está la Tarifa 1 A”.'
        ]);
    }
    ...
    if (!$stmt) {
        salir([
            'success' => 0,
            'msg' => 'Error preparando consulta de tarifas.'
        ]);
    }
    ...
    if ($total === 0) {
        ...
        salir([
            'success' => 0,
            'msg' => "No encontré tarifas activas que coincidan con <strong>$busqueda</strong>."
        ]);
    }
    ...
    while ($row = $res->fetch_assoc()) {
        ...
        $detalle .= "
            ...
            <span>Precio con IVA: <strong>" . dinero($precioConIva) . "</strong></span><br>
            <span>Precio sin IVA: <strong>" . dinero($precioSinIva) . "</strong></span><br>
    ...
    salir([
        'success' => 1,
        'respuesta' => "Encontré <strong>$total</strong> tarifa(s) que coinciden con <strong>$busqueda</strong>.",
        'detalle' => $detalle
    ]);
}
salir([
