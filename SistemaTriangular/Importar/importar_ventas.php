      ...
      $Relacion=$_SESSION[Relacion];
        ...
        for($i=0;$i<$sheetCount;$i++)
        {
            ...
            foreach ($Reader as $Row)
            {
                ...
                $ValorDeclarado = "";
                if(isset($Row[2])) {
                    $ValorDeclarado = mysqli_real_escape_string($con,$Row[2]);
                }
            

                    if (!empty($idProveedor)) {
                
                    $SQL="SELECT NdeCliente,nombrecliente,Relacion,Ciudad,Provincia,Direccion,idProveedor,Relacion,Latitud,Longitud FROM Clientes WHERE idProveedor ='".$idProveedor."' AND Relacion='".$_SESSION['Relacion']."'";
                    $DATOS_SQL=mysqli_query($con,$SQL);

                    $ROW=mysqli_fetch_array($DATOS_SQL);                    

                    $query ="INSERT INTO Clientes_importacion(`NdeCliente`,`nombrecliente`,`Relacion`,`Ciudad`,`Provincia`,`Direccion`,`Latitud`,`Longitud`)VALUES('".$ROW['idProveedor']."','".$ROW['nombrecliente']."','".$ROW['Relacion']."','".$ROW['Ciudad']."','".$ROW['Provincia']."','".$ROW['Direccion']."','".$ROW['Latitud']."','".$ROW['Longitud']."')";                 
...
<!DOCTYPE html>
    ...
    <head>
        <meta charset="utf-8" />
        <title>Sistema Caddy | Importa Ventas</title>
