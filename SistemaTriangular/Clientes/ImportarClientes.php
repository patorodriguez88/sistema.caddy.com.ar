...
session_start();
if($_GET[Inicio]=='si'){
unset($_SESSION[imp_razonsocial]);
unset($_SESSION[imp_ncliente]);
...
$csv_insert_table   = 'Clientes';     // Destination table for CSV files
...
<title>Caddy<?php echo (VERSION); ?></title>
...
skin_open();
echo ('<caption>Subir Archivos para la Importación de Nuevos Clientes</caption>');
skin_close();
...
if ($row = mysql_fetch_row($idoferta)) {
 $NCliente = trim($row[0])+1;
      ...
      if (($csv_insert_table != "") && (preg_match("/(\.csv)$/i",$curfilename)))
      {
        ...
        $dumpline = 'INSERT INTO '.$csv_insert_table.' (`NdeCliente`, `nombrecliente`,`Mail`,
        `Ciudad`, `Provincia`, `Pais`,`CodigoPostal`, `Telefono`,`Celular`,`Direccion`,`Observaciones`,
        `Relacion`,`idProveedor`,`Barrio`) VALUES ('.$NCliente.','.$dumpline.');';
