...
<body>
  ...
  if ($_GET['Orden'] == 'Agregar') {
    ...
    $sql = mysql_query("SELECT Cliente,CodigoProductos FROM Recorridos WHERE Numero='$Recorrido'");
    $ClienteEncontrado = mysql_fetch_array($sql);
    $Cliente = $ClienteEncontrado[Cliente];
  ...
  ?>
    ...
    //CODIGO DE PRODUCTOS      
    $sql = mysql_query("SELECT PrecioVenta,Iva FROM Productos WHERE Codigo='$Cliente[CodigoProductos]'");
    $fileServicios = mysql_fetch_array($sql);
    $PrecioRecorrido = $fileServicios['PrecioVenta'] / $fileServicios['Iva'];
    ...
    $sql = "INSERT INTO Logistica(
			...
			FechaVencSeguro,
			Observaciones,
			Estado,CombustibleSalida,
      idUsuarioChofer,
      Cliente,
      PrecioRecorrido) VALUES (
			'{$Numero}',
			'{$Fecha}',
			'{$Hora}',
			'{$Controla}',
...
'{$Orden}','{$CodigoSeguimiento}','{$row[id]}','{$NumeroRepo}','{$importevalorcobro}','{$row[id]}')");
                ...
                $Hora = date("H:i");
                $state = $Estado;
                $sql = mysql_query("SELECT idClienteOrigen,idClienteDestino,CodigoProveedor FROM TransClientes WHERE id='$row[id]");
      ...
      ?>
        <script>
          alertify.alert("LA ORDEN SE ENCUENTRA SIN CARGAR O YA SE ENCUENTRA CERRADA",
            function() {
              alertify.success("Intentanlo nuevamente");
            });
        </script>
      <?php
                }
