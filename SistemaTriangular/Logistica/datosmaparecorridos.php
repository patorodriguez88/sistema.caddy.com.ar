...
// $query="SELECT id,nombrecliente,Direccion,Ciudad,Recorrido,Relacion,idProveedor FROM TransClientes WHERE Recorrido IN($Recorrido)";
$query="SELECT id,nombrecliente,Direccion,Ciudad,Recorrido,Relacion,idProveedor FROM Clientes WHERE Recorrido IN($Recorrido)";

// $query ="SELECT * FROM `TransClientes` WHERE `Fecha` = '2019-07-12'";
...
$localizacion = $datosmapa[2];
  //si la funcion geolocalizar es true, y la direccion se modifico actualizo la tabla clientes con laitud y longitud
//     if($datosmapa['status']==='OK'){
//   $sqlBusca=mysql_query("SELECT id,Latitud,Longitud FROM Clientes WHERE id='$row[id]' AND Latitud='$latitud'");
//     if(mysql_num_rows($sqlBusca)==0){
//     $sql=mysql_query("UPDATE Clientes SET Latitud='$latitud',Longitud='$longitud' WHERE id='$row[id]'");
//     }
//   }
  ...
  echo 'type="' . parseToXML($row['Recorrido']) . '" ';
  echo '/>';
