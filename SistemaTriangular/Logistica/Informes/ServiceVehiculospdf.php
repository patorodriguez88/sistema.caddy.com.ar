...
function Header()
{
	...
	$Dominio=$_GET[IdVehiculo];
  $con = new DB;
	$historial = $con->conectar();	
  $Fecha=date('d/m/Y');
  $strConsulta =mysql_query("SELECT * FROM Vehiculos WHERE Dominio='$Dominio'");
  ...
  $VencimientoTarjetaVerde=$row[VencimientoTarjetaVerde]; 
  $Observaciones=$row[Observaciones];
  
  $idService=$_GET[IdService];
  $sqlService=mysql_query("SELECT * FROM ServiceVehiculos WHERE id='$idService';");
  $sqlRespuestaService=mysql_fetch_array($sqlService);
  
  $kmReales=$sqlRespuestaService[kmReales];
  $ServiceRealizado=$sqlRespuestaService[ServiceRealizado];
  $FechaServiceRealizado=$sqlRespuestaService[FechaServiceRealizado];
  $LugarService=$sqlRespuestaService[LugarService];
  $ObservacionesService=$sqlRespuestaService[Observaciones];
  $CostoService=$sqlRespuestaService[CostoService];
  
	$this->SetFont('Arial','',10);
	$this->Text(20,14,'Caddy.',0,'C', 0);
	$this->Text(20,19,'Cuit: 30-71534494-3',0,'C', 0);
	$this->Text(20,24,'Domicilio: Justiniano Posse 1236, Barrio Jardín ',0,'C', 0);
	$this->Text(20,29,'www.caddy.com.ar',0,'C', 0);
	
	//FECHA
	$this->Ln(20);
 	$this->SetFont('Arial','',10);
	$this->Text(130,14,'Service Numero: '.$idService,0,'C', 0);

	$this->Ln(20);
 	$this->SetFont('Arial','',10);
	$this->Text(130,19,'Fecha:'.$FechaServiceRealizado,0,'C', 0);

	//REMITO NUMERO
	$this->Ln(20);
 	$this->SetFont('Arial','',10);
	$this->Text(130,24,'Hora: '.$Hora,0,'C', 0);

	$this->Ln(20);
 	$this->SetFont('Arial','',10);
	$this->Text(130,29,'Controla: '.$Controla,0,'C', 0);

	$this->Ln(20);
 	$this->SetFont('Arial','B',9);
	$this->Text(40,47,'VEHICULO',0,'C', 0);
	$this->Text(140,47,'SERVICIO TECNICO',0,'C', 0);
...
//   $this->Text(122,55,'Fecha Service: '.$FechaServiceRealizado,0,'C', 0);
	$this->Text(122,55,'Service a Realizar: '.$ServiceRealizado,0,'C', 0);
	$this->Text(122,60,'Taller Service: '.$LugarService,0,'C', 0);
	$this->Text(122,65,'Costo Service: '.$CostoService,0,'C', 0);
  ...
  $this->SetFont('Arial','',10);
	...
	$this->SetFont('Arial','B',9);
	$this->Text(20,164,'MARQUE LAS OBSERVACIONES',0,'C', 0);
  $this->Image('../../images/auto.png',30,170,23);

	$this->Text(95,164,'ESPECIFIQUE LAS OBSERVACIONES DE CHAPA Y PINTURA',0,'C', 0);
...
// 	$this->SetHeights(array(100));
	...
	$this->SetFont('Arial','B',12);
	$this->Text(80,242,'OBSERVACIONES PARA EL TALLER',0,'C', 0);
...
// 	$this->SetFont('Arial','B',);
  $this->Row(array($ObservacionesService));
...
// 	//ACLARACION CLIENTE
	...
	$this->SetX(150);
	$this->SetFont('Arial','B',8);
	$this->Cell(300,100,'Firma Taller',0,0,'L');

	$this->SetY(-15);
	$this->SetFont('Arial','B',8);
	$this->Cell(100,10,'Service Flota Caddy',0,0,'L');
...
// 	$fila = mysql_fetch_array($pacientes);
  ...
  $pdf->Line(20, 94, 190, 94);  //Horizontal ESTADO DEL VEHICULO
	...
	$pdf->Line(90, 165, 90, 220);  //VERTICAL CUADRO
	$pdf->Line(90, 165, 190, 165);  //Horizontal CUADRO
	$pdf->Line(190, 165, 190, 220);  //VERTICAL CUADRO
	$pdf->Line(90, 220, 190, 220);  //Horizontal CUADRO
	
	$strConsulta =mysql_query("SELECT * FROM Variables");
	while($fila=mysql_fetch_array($strConsulta)){
    if($fila[Nombre]=='CostoPeajes'){
    $CADP = $fila[CostoPeajes];// Costo Actual de Peajes
    }elseif($fila[Nombre]=='PrecioNaftaSuper'){
  	$PNS = $fila[PrecioNaftaSuper];
    }
  }	
...
//---------------VEHICULOS----------------------------------------------------------------------------
//   $_SESSION['Dominio']=$_GET[IdDominio];
  $Dominio=$_GET[IdVehiculo];	
	...
	$pdf->Text(35,40,'INGRESO A TALLER VEHICULO: '.$Marca.' '.$Modelo.' ('.$Dominio.')', 0);

// 	$pdf->SetFont('Arial','B',10);
// 	$pdf->Text(20,247,$ObservacionesService,0,'C', 0);
