...
$query="SELECT Clientes.id,Clientes.Latitud,Clientes.Longitud,TransClientes.ClienteDestino,TransClientes.DomicilioDestino,TransClientes.id,TransClientes.CodigoSeguimiento 
FROM TransClientes 
inner join Clientes ON TransClientes.idClienteDestino=Clientes.id 
where TransClientes.FechaEntrega=CURDATE() AND Entregado=0 AND Devuelto=0 AND TransClientes.Eliminado=0 AND TransClientes.Recorrido=1027 ";
...
if(!isset($row['Latitud'])){
  ...
  $mysqli->query("UPDATE Clientes SET Latitud='".$latitud."',Longitud='".$longitud."' WHERE Clientes.id='".$row['id']."'");
