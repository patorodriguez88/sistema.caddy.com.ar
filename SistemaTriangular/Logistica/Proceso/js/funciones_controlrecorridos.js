  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (jsonData.estima == 1) {
        ...
        $(".text-nowrap.2")
          .removeClass("text-nowrap 2")
          .addClass("text-nowrap 2 text-danger")
          .html("Valores Estimados");
      } else {
      }
      $("#total_value_paq").html(
        currencyFormat(Number(jsonData.total_value_paq)),
      );
      $("#total_value_km").html(
        currencyFormat(Number(jsonData.total_value_km)),
      );
      ...
      if (jsonData.detalle_servicios && jsonData.detalle_servicios.length > 0) {
        jsonData.detalle_servicios.forEach(function (item, index) {
          let estado = item.EstadoEntrega || "Sin seguimiento";

          let badgeEstado =
            '<span class="badge bg-secondary">' + estado + "</span>";

          if (estado === "Entregado al Cliente" || estado === "Entregado") {
            badgeEstado =
          ...
          if (
            ...
            estado === "Devuelto" ||
            estado === "1ra Visita Fallida" ||
            estado === "2da Visita Fallida"
          ) {
            badgeEstado = '<span class="badge bg-danger">' + estado + "</span>";
          }
          ...
          htmlDetalle += `
            <tr>
                ...
                <td>${item.Destino}</td>
                <td>${badgeEstado}</td>
                <td>$ ${Number(item.PrecioCosto || 0).toLocaleString("es-AR", { minimumFractionDigits: 2 })}</td>
                <td>$ ${Number(item.PrecioVenta || 0).toLocaleString("es-AR", { minimumFractionDigits: 2 })}</td>
            </tr>
        `;
        });
      } else {
        ...
        <tr>
            <td colspan="6" class="text-center text-muted">
                No se encontraron servicios para esta orden.
            </td>
        </tr>
    `;
      }

      $("#detalle_servicios_body").html(htmlDetalle);
    },
  });
}
...
function init_datatable(fechas) {
  ...
  $("#header_flota").html(
    "Recorridos Fechas Desde " + new_date2 + " Hasta " + new_date_2,
  );
    ...
    columns: [
      {
        ...
        render: function (data, type, row) {
          ...
          return (
            ...
            Fecha +
            "</td></br>" +
            '<small class="mr-2" style="font-size:8px"><span class="mdi mdi-clock"></span> ' +
            row.Hora +
            " </small>"
          );
        },
      },
      {
        ...
        render: function (data, type, row) {
          return (
            ...
            row.Modelo +
            "</td></br>" +
            '<small class="mr-2" style="font-size:8px"><span class="mdi mdi-truck"></span> ' +
            row.Patente +
            " | " +
            row.NombreChofer +
            "-" +
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          return (
            ...
            row.Nombre +
            "</td></br>" +
            '<small class="mr-2" style="font-size:8px"><span class="mdi mdi-map-marker text-' +
            color +
            '"></span> Rec. N: ' +
            row.Recorrido +
            " Estado: " +
      ...
      },
      {
        ...
        render: function (data, type, row) {
          return (
            "<td>" +
            "<i style='cursor:pointer' class='mdi mdi-18px mdi-go-kart-track text-success' onclick='initMap_order(" +
            row.id +
            ")'></i>" +
            "<i style='cursor:pointer' class='mdi mdi-18px mdi-information-outline text-warning btn-modal-info' onclick='info_order(" +
            row.id +
            ")'></i>" +
            "</td>"
          );
