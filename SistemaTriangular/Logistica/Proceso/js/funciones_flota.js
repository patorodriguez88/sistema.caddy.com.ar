        ...
        columns: [
            ...
            render: function (data, type, row) {
               ...
               var color='warning';      
                ...
                if(row.Retirado==1){
                    return '<td>Entrega'+ 
                           '<h6><span class="badge badge-'+color+' text-white">'+row.Estado+'</span></h6></td>';
                }else{
                    return '<td>Retiro';  
                           '<h5><span class="badge badge-'+color+' text-white">'+row.Estado+'</span></h5></td>';
                    }
    ...
    ajax: {
        ...
        columns: [
            ...
            render: function(data, type, row, meta) {
                return type === 'display' ?
                    '<div class="progress progress-lg"><div class="progress-bar bg-warning" role="progressbar" style="width: '+ (row.Cerradas/row.Total)*100+ '%" aria-valuenow="'+ row.Cerradas +'" aria-valuemin="0" aria-valuemax="'+row.Total+'"></div></div>' :
                    data;
