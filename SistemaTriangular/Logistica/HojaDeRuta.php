...
// Hoja de Ruta). Esta (sin número) y HojaDeRuta1.php no están enlazadas en
...
//             alert('Geocode was not successful for the following reason: ' + status);
        ...
        _googleAcDebounce(inputstart, {
          onSelect: function(place) {
            ...
            if (retiro == 0) { calculateAndDisplayRoute(directionsService, directionsRenderer); }
            else {
              document.getElementById('map').style.height = '100%';
              var geocoder = new google.maps.Geocoder;
              geocoder.geocode({ 'address': inputstart.value }, function(results, status) {
          ...
          onSelect: function(place) {
            ...
            place.address_components.forEach(function(c) {
              ...
              if (t === 'administrative_area_level_1') {
                provincia = c.long_name;
                if (provincia !== 'Córdoba') {
                  alertify.error('La Provincia de destino debe ser Córdoba, no ' + provincia);
                  end.value = ''; end.focus(); return;
                }
              } else if (t === 'locality') { document.getElementById('Ciudadorigen_t').value = c.long_name; }
              else if (t === 'postal_code') { document.getElementById('Codigo_Postalorigen_t').value = c.short_name; }
              else if (t === 'street_number') { document.getElementById('numeroorigen_t').value = c.long_name; }
              else if (t === 'route') { document.getElementById('calleorigen_t').value = c.long_name; }
            });
            if (retiro == 0) { calculateAndDisplayRoute(directionsService, directionsRenderer); }
            else {
              document.getElementById('map').style.height = '100%';
              var geocoder = new google.maps.Geocoder;
              geocoder.geocode({ 'address': end.value }, function(results, status) {
                if (status == 'OK') { map.setCenter(results[0].geometry.location); new google.maps.Marker({ map: map, position: results[0].geometry.location }); }
              });
            }
          }
