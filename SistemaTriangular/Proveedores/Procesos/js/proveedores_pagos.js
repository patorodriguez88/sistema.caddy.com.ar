...
$("#cargar_pago_btn_ok_n").click(function () {
  ...
  var datos = {
    ...
    num_cheque_propio: num_cheque_propio,
    fecha_cheque_propio: fecha_cheque_propio,
    banco_cheques_propio: banco_cheque_propio,
  };
        ...
        success: function (jsonData) {
          if (jsonData.success == "1") {
            ...
            $.ajax({
              ...
              type: "post",
              dataType: "json",
              success: function (jsonData_asiento) {
                if (jsonData_asiento.success == "1") {
                  $("#nasiento_p").val(jsonData_asiento.Asiento);
                }
              },
              error: function (xhr) {
                console.error("Error al obtener el asiento:", xhr.responseText);
              },
            });
          }
        },
        error: function (xhr) {
          console.error("Error al obtener el proveedor:", xhr.responseText);
        },
      });
    },
  });
...
$("#cargar_pago_btn_continuar").click(function () {
  ...
  if (valor_saldo > 0) {
    ...
    if (checked_anticipos.length == 0) {
      ...
      var datos = {
        PagoDesdeAnticipos: 1,
        RazonSocial: RazonSocial,
        Cuit: Cuit,
        idProveedor: idproveedor,
        idFacturas: checked_facturas,
        idAnticipos: checked_anticipos,
        ...
        },
        {
          ...
          render: function (data, type, row) {
            return (
              ...
              '" value="' +
              row.Haber +
              '" type="checkbox" class="form-check-input dt-checkboxes" onclick="sumar_anticipos()" >' +
