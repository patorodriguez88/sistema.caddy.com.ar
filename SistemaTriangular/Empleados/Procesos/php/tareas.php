        ...
        if ($handle = opendir($folder_path)) {
            ...
            while (false !== ($file = readdir($handle))) {
                ...
                if ($file != "." && $file != "..") {
                    ...
                    echo    '</a>';
                    echo    '<a href="/SistemaTriangular/' . $folder_path . '/' . $file . '" class="btn btn-link btn-lg text-muted">';
                    echo    '<i class="dripicons-download"></i>';
                    echo    '</a>';
                    echo '</div>';
...
if (isset($_POST['Tareas_comentarios'])) {
    ...
    $gidAsana = $_POST['gid_asana'] ?? '';

    // Llamada a la API/función que trae los comentarios
    $datos = Get_storys($gidAsana);

    // Normalizar: puede venir como string JSON o como array PHP.
    if (is_string($datos)) {
    ...
    // Extraer lista de comentarios si existe
    $lista = $datos['data'] ?? [];
...
if (isset($_POST['NombreUsuario'])) {
    ...
    if (!empty($_POST['gid'])) {
        ...
        $sql = $mysqli->query("SELECT CONCAT(Nombre,' ',Apellido)as Nombre,Usuario FROM usuarios WHERE usuario='" . $usuario . "'");
    }
