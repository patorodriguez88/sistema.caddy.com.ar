...
// $sql=mysql_query("SELECT Patente,NombreChofer FROM Logistica WHERE Recorrido='$Recorrido' AND Estado IN('Cargada','Alta') AND Eliminado='0'");
...
// echo '<markers>';
// $query=mysql_query("SELECT * FROM HojaDeRuta WHERE Entregado=  Eliminado='0' ORDER BY Posicion ASC");	  
$sqltransclientes=mysql_query("SELECT RazonSocial,ClienteDestino,DomicilioOrigen,LocalidadOrigen,Retirado,CodigoSeguimiento FROM TransClientes WHERE Entregado='0'  AND Eliminado='0' LIMIT 1,5");	
// $dato=mysql_fetch_array($sqltransclientes);

while ($row = mysql_fetch_array($sqltransclientes)){
$sqlseguimiento=mysql_query("SELECT Estado FROM Seguimiento WHERE id=(SELECT MAX(id)as id FROM Seguimiento WHERE CodigoSeguimiento='$row[CodigoSeguimiento]')");
...
if($row['Retirado']==1){
  ...
  $sql = mysql_query("SELECT count(id)as repe FROM TransClientes WHERE RazonSocial = '$row[RazonSocial]' AND Recorrido='$row[Recorrido]' AND Eliminado='0'");  
...
$sqlid=mysql_query("SELECT idProveedor FROM Clientes WHERE id='$row[IngBrutosOrigen]'");
$rowidcliente=@mysql_fetch_array($sqlid);  

echo json_encode(array('location'=>array('lat'=>$latitud,'lng'=>$longitud),"stopover"=>true)); 
