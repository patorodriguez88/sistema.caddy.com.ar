...
// if($_POST[Descontar]==1){
  ...
  $sqlbuscar=$mysqli->query("SELECT NumeroComprobante,Debe,idClienteOrigen FROM TransClientes WHERE id='$OrdenN[$i]' AND TipoDeComprobante='Remito' AND Eliminado='0'");
  $Dato= $sqlbuscar->fetch_array(MYSQLI_ASSOC);
  $Actualizar_CtasCtes="UPDATE Ctasctes SET Debe='$Dato[Debe]' WHERE idCliente='$Dato[idClienteOrigen]' AND NumeroVenta='$Dato[NumeroComprobante]' AND Haber='0' LIMIT 1";
