...
// `Fecha`, `TipoDeComprobante`, `NumeroComprobante`, `CompraMercaderia`, `Debe`, `Haber`, `ClienteDestino`, 
//     `DocumentoDestino`, `DomicilioDestino`, `LocalidadDestino`, `IngBrutosDestino`, `TelefonoDestino`, 
//     `CodigoSeguimiento`, `NumeroVenta` `LocalidadOrigen`, `IngBrutosOrigen`,
//     `Cantidad`, `Usuario`, `Entregado`, `FormaDePago`, `EntregaEn`, `Eliminado`, `CodigoProveedor`, `Observaciones`, `Transportista`, `Recorrido`,
//     `ProvinciaDestino`, `ProvinciaOrigen`, `Facturado`, `ComprobanteF`, `NumeroF`, `Kilometros`, `FechaEntrega`,`Redespacho`, 
//     `Retirado`, `CobrarEnvio`    
...
$color2='white';
$font2='black';
$sqlTransClientes=mysql_query("SELECT * FROM TransClientes WHERE CodigoSeguimiento='$_GET[CS]' AND Eliminado='0'");
$DatoTransClientes=mysql_fetch_array($sqlTransClientes);
    
$DomicilioOrigen=	$DatoTransClientes[DomicilioOrigen];
$_SESSION['DomicilioEmisor_t']=$DomicilioOrigen;//Domicilio
$SituacionFiscalOrigen=	$DatoTransClientes[SituacionFiscalOrigen];
$_SESSION['SituacionFiscalEmisor_t']=$SituacionFiscalOrigen;//SituacionFiscal
$TelefonoOrigen=$DatoTransClientes[TelefonoOrigen];
$_SESSION['TelefonoEmisor_t']=$TelefonoOrigen;//Telefono
$LocalidadOrigen=$DatoTransClientes[SituacionFiscalOrigen];
$_SESSION['LocalidadOrigen_t']=$LocalidadOrigen;//Telefono
$CelularOrigen=$DatoTransClientes[TelefonoOrigen];//Telefono

$idClienteDestino=$DatoTransClientes[idClienteDestino];
$_SESSION['idClienteDestino_t']=$idClienteDestino;
$DomicilioDestino=$DatoTransClientes[DomicilioDestino];
$_SESSION['DomicilioDestino_t']=$DomicilioDestino;//Domicilio
$SituacionFiscalDestino=$DatoTransClientes[SituacionFiscalDestino];
$_SESSION['SituacionFiscalDestino_t']=$SituacionFiscalDestino;//SituacionFiscal
$TelefonoDestino=$DatoTransClientes[TelefonoDestino];
$_SESSION['TelefonoDestino_t']=$TelefonoDestino;//Telefono
$CelularDestino=$DatoTransClientes[TelefonoDestino];
$_SESSION['CelularDestino_t']=$CelularDestino;//Telefono
$LocalidadDestino=$DatoTransClientes[LocalidadDestino];
$_SESSION['LocalidadDestino_t']=$LocalidadDestino;//Telefono
		
$cliente=$DatoTransClientes[RazonSocial];//LocalidadDestino
$_SESSION['NombreClienteA']=$cliente;
$CuitClienteA=$DatoTransClientes[Cuit];//
$_SESSION['NCliente']=$CuitClienteA;
$CuitDestino=$DatoTransClientes[DocumentoDestino];//
$_SESSION['NClienteDestino_t']=$CuitDestino;	//CUIT
$ClienteDestino=$DatoTransClientes[ClienteDestino];
$_SESSION['NombreClienteDestino_t']=$ClienteDestino;//NOMBRE CLIENTE
...
$BuscaRepoAbierta="SELECT NumPedido,NumeroRepo FROM Ventas WHERE NumPedido='$_GET[CS]' AND Eliminado='0'";
...
$ordenar="SELECT *, SUM(Total) as Total FROM Ventas NumPedido='$_GET[CS]' AND Eliminado=0 GROUP BY NumeroRepo  ORDER BY NumeroRepo ASC";	
...
if($datosql[Web]==0){
$Redespacho=1;  // CAMBIAR A 1 CUANDO ESTE SEGURO QUE ESTAN OK TODAS LAS LOCALIDADES DE LOS CLIENTES
}else{
$Redespacho=0;   
}
...
$Ordenar1="SELECT * FROM Ventas WHERE NumPedido='$_GET[CS]' AND Eliminado=0";
...
$result=mysql_query("SELECT SUM(Cantidad*Precio) as Saldo FROM Ventas WHERE NumPedido='$_GET[CS]' AND Eliminado=0");
...
echo "<tr>";
  ...
  echo "<input type='hidden' value='1' name='editor'>";
...
$Vacio=mysql_query("SELECT * FROM Ventas WHERE Cliente='$cliente' AND fechaPedido=curdate() AND terminado=0 AND Usuario='".$_SESSION['Usuario']."'");
    ...
    $TelefonoDestino=$_SESSION['TelefonoDestino_t']." - ".$_SESSION['CelularDestino_t'];
      ...
      if($_GET[retiro_t]==0){
		...
		$Sqltransportista=mysql_query("SELECT * FROM Logistica WHERE Recorrido='$Recorrido' AND Estado='Cargada'");
