...
if(isset($_POST['Sla'])){
  ...
  // $_SESSION['RecorridoMapa']=$_POST['Recorrido'];
  $sql="SELECT * FROM TransClientes WHERE Fecha>='$_POST[Inicio]' AND Fecha<='$_POST[Final]' AND  Eliminado=0 AND Haber=0 AND  CodigoSeguimiento<>''";
...
//ACTUALIZA ROADMAP
$sql=$mysqli->query("UPDATE `Roadmap` SET Estado='Cerrado' WHERE Seguimiento='$sqldato[CodigoSeguimiento]'");
...
if(isset($_POST['EliminarRegistro'])){
  ...
  if($sql=$mysqli->query("UPDATE `TransClientes` SET Eliminado='1',Usuario='Elimino $_SESSION[Usuario]' WHERE id='$_POST[id]' LIMIT 1")){
  $transclientes=1;    
  }else{
  $transclientes=0;  
  }
