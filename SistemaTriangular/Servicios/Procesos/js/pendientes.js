(function () {
  // Carga recorridos al abrir el modal
  var modalRec = document.getElementById("standard-modal-rec");
  if (modalRec) {
    modalRec.addEventListener("shown.bs.modal", function () {
      var select = document.getElementById("recorrido_t");
      if (!select) return;

      // Placeholder mientras carga
      select.innerHTML = '<option value="">Cargando recorridos...</option>';

      // Tomamos el código de seguimiento si estuviera seteado en el hidden
      var csInput = document.getElementById("cs_modificar_REC");
      var cs = csInput ? csInput.value || "" : "";

      // Llamada al backend que devuelve <option>...</option>
      // Ajustá la URL si tu endpoint difiere
      fetch("Procesos/php/pendientes.php", {
        method: "POST",
        headers: {
          "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
        },
        body: new URLSearchParams({ BuscarRecorridos: 1, cs: cs }).toString(),
      })
        .then(function (r) {
          return r.text();
        })
        .then(function (htmlOptions) {
          // El PHP devuelve opciones crudas
          select.innerHTML =
            htmlOptions && htmlOptions.trim()
              ? htmlOptions
              : '<option value="">No hay recorridos activos</option>';
        })
        .catch(function (err) {
          console.error("Error cargando recorridos:", err);
          select.innerHTML =
            '<option value="">Error al cargar recorridos</option>';
        });
    });
  }
})();
...
function modificarrecorrido(i) {
  ...
  window._csRec = i; // respaldo global por si el hidden no existe aún
  $("#standard-modal-rec").attr("data-cs", i); // backup en el modal
  $.ajax({
...
$("#modificarrecorrido_ok").on("click", async function () {
  const $btn = $(this);

  // Obtener cs desde varias fuentes (como pusiste)
  const cs =
    ($("#cs_modificar_REC").length ? $("#cs_modificar_REC").val() : "") ||
    $("#standard-modal-rec").attr("data-cs") ||
    window._csRec ||
    "";

  const r = $("#recorrido_t").val();
  ...
  try {
    const resp = await fetch("../Funciones/Funciones.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ActualizaRecorrido: 1, cs, r }),
    });

    // Si vino HTTP error, intento leer texto para mostrar algo útil
    if (!resp.ok) {
      const raw = await resp.text().catch(() => "");
      throw new Error(raw || `HTTP ${resp.status} - ${resp.statusText}`);
    }

    // Intento parsear JSON, si falla pruebo texto crudo
    let data;
    try {
      ...
      if (raw.startsWith("{") || raw.startsWith("[")) {
        data = JSON.parse(raw);
      } else if (raw.toUpperCase() === "OK") {
        data = { success: 1, message: "OK" };
      } else {
        // intento extraer success/message si vino mezclado con warnings
        const mSucc = raw.match(/"success"\s*:\s*([01])/);
        const mMsg = raw.match(/"message"\s*:\s*"([^"]*)"/);
        data = mSucc
          ? { success: parseInt(mSucc[1], 10), message: mMsg ? mMsg[1] : "" }
          : { success: 0, message: raw || "Respuesta no válida" };
      }
    }

    if (Number(data.success) === 1) {
      // Recargar DataTable sin perder paginación
      const dt = $("#seguimiento").DataTable();
      ...
      Swal.fire({
        ...
        text: data.message || "No pudimos actualizar el Recorrido.",
      });
    }
  } catch (err) {
    ...
    Swal.fire({
      ...
      text:
        String(err?.message || err) || "No se pudo conectar con el servidor.",
    });
  } finally {
