var datatable = $('#seguimiento').DataTable({
  ...
  ajax: {
       url:"Procesos/php/redespachos.php",
       data:{'Pendientes':1},
       processing: true,
       type:'post',
      },
      columns: [
         ...
         return '<td><b>'+row.RazonSocial+'</br>'+  
          ...
          render: function (data, type, row) {
               ...
               '<a style="cursor:pointer" data-id="' + row.CodigoSeguimiento + '" id="'+row.CodigoSeguimiento+'"><b class="text-primary">'+row.Recorrido+'</b></a>'+
               '</td>';
            }
           },         
          ...
          var servicio='Origen';  
              ...
              'data-title="' + data.ClienteDestino + '" data-fieldname="' + data + '"><b>' + row.CodigoSeguimiento + '</b></a></br>'+
              '<a><b>'+servicio+'</b></a><br/>'+
          ...
          render: function (data, type, row) {
             return '<td>'+
             '<a><b class="text-success">'+row.ProveedorNombre+'</b></a></br>'+
             '<a><b class="text-muted">'+row.ProvTel+' '+row.ProvCel+'</b></a></br>'+
             '</td>';
          }
         },    
          {data:"CodigoSeguimiento",
          render: function (data, type, row) {
            return '<td class="table-action col-xs-3">'+
            '<a class="text-success"><b>' + row.CodigoRedespacho + '</b></a></br>'+
            '<a><b>'+row.ObsR+'</b></a><br/>'+
            '<span class="badge badge-dark-lighten"> $ '+row.Importe+'</span>'+  
            '</td>';
          }
        },
        ...
        //       '<a data-id="' + row.id + '" id="'+row.id+'" onclick="eliminar(this.id);" class="action-icon"> <i class="mdi mdi-delete text-warning"></i></a>'+
        //       '</td>';
        //     }
        //   },
      ...
      columns: [
        ...
        {data:"idPedido",
         render: function (data, type, row) {
          ...
          '<a id="'+row.idPedido+'" onclick="eliminarVentas(this.id);" class="action-icon"> <i class="mdi mdi-trash-can text-danger"></i></a>'+
...
$.ajax({
    data:{'ModificarDatosVentas':1,'idPedido':idPedido,'codigo':codigo,'titulo':titulo,'total':total,'idTrans':idTrans,'precio':precio,'cantidad':cantidad},
    url:'Procesos/php/funciones.php',
    type:'post',
    success: function(response)
     {
