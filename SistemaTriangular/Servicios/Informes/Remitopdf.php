...
while ($file = readdir($filehandle)) {
        ...
        if ($file != "." && $file != "..") {
            ...
            $pdf->Image($ruta.''.$file,$w,152,15,15);
        $w=$w+25;    
        }
...
//         $ImporteNeto=(($row[Total])/1.21);
        $ImporteNeto=(($filaVentas[Cantidad]*$filaVentas[Precio])/1.21);
...
//         $Seguro_total_label=number_format($Seguro_total,2,',','.');  
      ...
      if($i%2 == 1)
        ...
        $pdf->SetFillColor(255,255,255);  
        $pdf->SetTextColor(0);
				$pdf->Row(array($filaVentas['Codigo'], $filaVentas['Titulo'], $filaVentas['Comentario'], $filaVentas['Cantidad'],'$ '.$ImporteNeto_label,'$ '.$Iva_precio_label,$Precio));
			}
			else
			{
        $pdf->SetFont('Arial','B',5);
...
//   $pdf->SetTextColor(0,0,0);//color negro
  ...
  $pdf->SetY(-90);
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(100,10,'Firma Cliente Retiro',0,0,'L');
	//ACLARACION CLIENTE
	$pdf->SetY(-90);
  $pdf->SetX(90);

	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(100,10,'Aclaracion Cliente Retiro',0,0,'L');
	//D.N.I.
	$pdf->SetY(-90);
	$pdf->SetX(150);
  $pdf->SetFont('Arial','B',8);
	$pdf->Cell(100,10,'D.N.I. Cliente Retiro',0,0,'L');
