...
function Header()
{
    ...
    $NumeroRepo=$_GET['NR'];
    $ClienteOrigen=$_SESSION[clienteorigen_t];
    $con = new DB;
    $historial = $con->conectar();	

    $this->SetFont('Arial','',10);
 	$this->Image('../../images/caddy.jpg' , 100 ,8, 50 , 22,'JPG', '');
  ...
  //RESUMEN
if($Repartidor=='Todos'){
  if($ClienteOrigen=='Todos'){
    $sqlsumarentregas=mysql_query("SELECT id,CodigoSeguimiento FROM TransClientes WHERE FechaEntrega='$Fecha' AND Eliminado=0");  
    }else{
    $sqlsumarentregas=mysql_query("SELECT id,CodigoSeguimiento FROM TransClientes WHERE RazonSocial='$ClienteOrigen' AND FechaEntrega='$Fecha' AND Eliminado=0");
  }
...
$ef=number_format(($e/$sumarentregas)*100,2,',','.');  
  ...
  $this->SetFont('Arial','',10);
//  	$this->Image('../../images/caddy.jpg' , 120 ,8, 60 , 22,'JPG', '');
...
//ESTE CODIGO ES PARA BUSCAR POR PRODUCTO 
// $sql="SELECT TransClientes.id,CodigoSeguimiento FROM TransClientes INNER JOIN Asignaciones ON TransClientes.CodigoProveedor=Asignaciones.idProveedor 
// WHERE IngBrutosOrigen='36' AND FechaEntrega='2022-03-06' AND Eliminado=0 AND Asignaciones.CodigoProducto='5131' AND Asignaciones.Fecha='2022-03-06'";
...
if($ClienteOrigen=='Todos'){
  ...
  $sqlsumarentregas=mysql_query("SELECT id,CodigoSeguimiento FROM TransClientes WHERE RazonSocial='$ClienteOrigen' AND FechaEntrega='$Fecha' AND Recorrido='$recorrido' AND Eliminado=0");
  }  
...
// echo "</div>";  
  ...
  $FechaEntregab=$FechaEntrega[2]."/".$FechaEntrega[1]."/".$FechaEntrega[0];
    ...
    $this->SetWidths(array(12, 15,15, 40, 45, 13,18,13,23,35,15));
...
//DESDE ACA CUADRO RESUMEN
  ...
  if($Repartidor=='Todos'){
      ...
      if($_SESSION[clienteorigen_t]=='Todos'){
      $strConsulta="SELECT * FROM TransClientes WHERE Recorrido='$Recorrido' AND FechaEntrega='$Fecha' AND Recorrido='$Recorrido' AND Eliminado=0 ORDER BY idClienteDestino";
      }else{
