...
echo '<markers>';
$query=mysql_query("SELECT * FROM HojaDeRuta WHERE Seguimiento='$CodigoSeguimiento' AND  Eliminado='0' ORDER BY Posicion ASC");	  
while ($row = mysql_fetch_array($query)){
$sqltransclientes=mysql_query("SELECT RazonSocial,ClienteDestino,DomicilioOrigen,LocalidadOrigen,Retirado FROM TransClientes WHERE Estado='Abierto' AND CodigoSeguimiento='$row[Seguimiento]' AND Eliminado=0");	
...
if($dato['Retirado']==1){
  ...
  $sql = mysql_query("SELECT count(id)as repe FROM TransClientes WHERE RazonSocial = '$dato[RazonSocial]' AND Recorrido='$row[Recorrido]' AND Eliminado=0 ");  
