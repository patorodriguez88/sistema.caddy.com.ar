...
// 	}
  ...
  $arrayfecha=explode('-',$_GET['Hasta'],3);
	...
	$this->Text(80,43,'SUMAS Y SALDOS NO OPERATIVOS',0,'C', 0);
...
function Footer()
{
	...
	$this->Cell(100,10,'Sumas y Saldos No Operativos',0,0,'L');
  ...
  Eliminado=0 AND Fecha>='$Desde' AND Fecha<='$Hasta' AND Pendiente=0  GROUP BY Cuenta";
  ...
  $pdf->SetTextColor(255);
	...
	$strConsulta = "SELECT Cuenta,NombreCuenta,SUM(Debe)AS Debe,SUM(Haber)AS Haber FROM Tesoreria WHERE Eliminado=0 AND Pendiente=0 AND Fecha>='$Desde' AND Fecha<='$Hasta'  GROUP BY Cuenta";
  ...
  setlocale(LC_ALL,'es_AR');
	...
	$Muestra=mysql_query("SELECT SUM(Debe) as TotalDebe,  SUM(Haber) as TotalHaber FROM Tesoreria WHERE Eliminado=0 AND Pendiente=0 AND Fecha>='$Desde' AND Fecha<='$Hasta' ");
