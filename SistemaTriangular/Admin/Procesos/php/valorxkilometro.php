...
// --------- Listar segmentos ----------
if ($action === 'listar') {
    $sql = "SELECT id, Segmento, Nombre, ValorKm, ValorHora, MaxKg, MaxM3, Activo FROM ValorxKilometro ORDER BY Segmento ASC";
...
if ($action === 'guardar') {
    ...
    $valorHora = isset($_POST['ValorHora']) ? (float)str_replace(',', '.', $_POST['ValorHora']) : 0;
    ...
    if ($id > 0) {
        $sql = "UPDATE ValorxKilometro SET Segmento=?, Nombre=?, ValorKm=?, ValorHora=?, MaxKg=?, MaxM3=?, Activo=? WHERE id=? LIMIT 1";
        if (!($stmt = $mysqli->prepare($sql))) {
            jexit(['ok' => false, 'error' => 'Prepare failed: ' . $mysqli->error]);
        }
        $stmt->bind_param('isddddii', $segmento, $nombre, $valorKm, $valorHora, $maxKg, $maxM3, $activo, $id);
    } else {
        $sql = "INSERT INTO ValorxKilometro (Segmento, Nombre, ValorKm, ValorHora, MaxKg, MaxM3, Activo) VALUES (?, ?, ?, ?, ?, ?, ?)";
        if (!($stmt = $mysqli->prepare($sql))) {
            jexit(['ok' => false, 'error' => 'Prepare failed: ' . $mysqli->error]);
        }
        $stmt->bind_param('isddddi', $segmento, $nombre, $valorKm, $valorHora, $maxKg, $maxM3, $activo);
    }
