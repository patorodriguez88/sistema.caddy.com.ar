...
// Empty CSV table if requested
  ...
  { 
    ...
    if (!TESTMODE && !$mysqli->query(trim($query), $dbconnection))
...
while($row=$sql->fetch_array(MYSQLI_ASSOC)){
$sqlclienteOrigen=$mysqli->query("SELECT * FROM Clientes WHERE id='$NCliente'");
$datoclienteOrigen=$sqlclienteOrigen->fetch_array(MYSQLI_ASSOC);
...
//AGREGO LOS DATOS PENDIENTES
$Fecha=date('Y-m-d');  
$updatepreventa=$mysqli->query("UPDATE PreVenta SET 
Fecha='$Fecha',
RazonSocial='$RazonSocial',
NCliente='$NCliente',
TipoDeComprobante='$TipoDeComprobante',
