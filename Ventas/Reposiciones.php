...
<!-- 		<script src="../scripts/ac_runactivecontent.js" type="text/javascript"></script> -->
    ...
    <script type="text/javascript">
      function lineacobro(){
          var importe = document.getElementById('valorcobro1').value;
          var importe1=((Number(importe)*4)/100);
          document.getElementById('importevalorcobro').value=importe;
          $("#cuerpocobro").html("");
          var tr = `<tr align='left' style='font-size:12px;'>
          <td>`+'CODIGO'+`</td>
          <td>`+'CARGO SERVICIOS ESPECIALES'+`</td>
          <td>`+'CARGO DE 4% SOBRE VALOR TRANSPORTADO ($ '+importe+')'+`</td>
          <td>$ `+ importe1 +`</td>
          <td>`+'1'+`</td>
          <td>$ `+ importe1+`</td>
          </tr>`;
          $("#cuerpocobro").append(tr);
          var total=document.getElementById('total_r').value;  
          var totalrepo=Number(importe1)+Number(total);  
          $('#total_repo').html('Total: $ '+totalrepo);
      }
      function vercobro(){
        var cobro=document.getElementById('cobro');
        if(cobro.checked==true){
          document.getElementById('valorcobro').style.display='block';  
          }else{
          document.getElementById('valorcobro').style.display='none';  
          document.getElementById('importevalorcobro').value='';
          $("#cuerpocobro").html("");
          }
      }
      function calculo(){
...
echo "<input type='hidden' id='total_r' value='$rowresult[Saldo]'>";
...
echo "<tbody id='cuerpocobro'></tbody>";
echo "</tr><tr style='background:red; color:white; font-size:16px;'><td align='right' colspan='6' style='font-size:16px'><strong id='total_repo'>Total: $Total</strong></td><td></td></tr>";
echo "</table>";
...
//DETERMINO LA OPCION PREDETERMINADA EN CLIENTES CAMPO Retiro
if($_SESSION['RetiroOrigen_t']==0){
$retiro='selected';
$retiro1='';  
}elseif($_SESSION['RetiroOrigen_t']==1){
$retiro='';
$retiro1='selected';    
}
...
echo "</select></div>";
//MODALIDAD DE PAGO
echo "<div style='margin-bottom:5px'><label style='margin-left:150px'>Modalidad de Cobro del Envío:</label><select name='cobranzadelenvio_t' style='width:400px;' size='1'>";
echo "<option value='1'>Contra Retiro o Entrega del Envío</option>";
echo "<option value='0'>Otra Forma de Pago</option>";
echo "</select></div>";
...
echo "<div><label style='margin-bottom:0px;margin-left:150px'>Recorrido:</label><input type='text' name='recorrido_t' style='width:400px;' list='recorrido_t'>";
...
echo "<option value='0'$retiro>Retiro y Entrega</option>";
echo "<option value='1'$retiro1>Solo Entrega</option>";
echo "</select></div>";
echo "<div><label style='margin-left:150px'>Cobro a cuenta y orden del cliente:</label><input id='cobro' type='checkbox' value='' name='cobro' Onclick='vercobro()'></div>";
echo "<div id='valorcobro' style='display:none;margin-left:150px'><label>Importe a Cobrar:</label><input name='valorcobro1' style='float:right;border:0px;width:400px;height:30px' type='input' id='valorcobro1' placeholder='$' Onblur='lineacobro()'></div>";
echo "<input type='hidden' id='importevalorcobro' name='importevalorcobro' value='' step='0.01' >";
...
if ($_GET['SolicitaEnvio']=="Confirmar Envio"){
    ...
    // SI HAY QUE COBRAR POR CUENTA Y ORDEN CARGO EL IMPORTE EN VENTAS
    if($_GET[importevalorcobro]<>''){
    $importevalorcobro=number_format((($_GET[importevalorcobro]*4)/100),2,'.','');  // ACA CALCULO EL 4%
    $totalimportevalorcobro=$importevalorcobro;  
    $sqlvalorcobro="INSERT INTO Ventas(Codigo,fechaPedido,Titulo,Precio,Cantidad,Total,Cliente,NumeroRepo,
    ImporteNeto,Iva1,NumPedido,Usuario,CobrarEnvio)
    VALUES('0000000000','{$Fecha}','CARGO SERVICIOS ESPECIALES','{$importevalorcobro}','1','{$totalimportevalorcobro}','{$cliente}',
    '{$NumeroRepo}','{$importevalorcobro}','{$ivaimportevalorcobro}','{$Codigo}','{$Usuario}','{$importevalorcobro}')";
    mysql_query($sqlvalorcobro);
    }
		...
		$result=mysql_query("SELECT SUM(Total) as Saldo FROM Ventas WHERE Cliente='$cliente' AND fechaPedido=curdate() AND terminado='0' AND Usuario='".$_SESSION['Usuario']."';");
		$rowresult = mysql_fetch_array($result);
...
// 		$cliente=$_SESSION['NombreClienteA'];
    ...
    $importevalorcobro=$_GET[valorcobro1];    
    ...
    idClienteDestino,CobrarEnvio,CobrarCaddy,ValorDeclarado)
		...
		'{$Transportista}','{$Recorrido}','{$ProvinciaDestino}','{$ProvinciaOrigen}','{$Retirado}','{$idClienteDestino}','{$importevalorcobro_label}',
    '{$_GET[cobranzadelenvio_t]}','{$ValorDeclarado}')";
    ...
    '{$NumeroDeOrden}','{$Codigo}','{$idClienteDestino}','{$NumeroRepo}','{$importevalorcobro}','{$idTransClientes}')";
...
//     $maxid=$datosqlhdr[id];
      ...
      if($Total<>0){
      mysql_query($IngresaCtasctes); //AGREGAR A CUENTA CORRIENTE
      }
