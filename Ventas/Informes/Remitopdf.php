...
//   $this->Image('../../images/LogoCaddy.png',10,8,22);  
  ...
  $CodigoSeguimiento=$_GET['CS'];
	...
	$this->Ln(20);
 	$this->SetFont('Arial','',10);
	$this->Text(150,24,'Guia de Carga N: '.$Repo,0,'C', 0);
...
// Observaciones Cliente Destino
  ...
  if($fila[Retirado]==0){
	$pdf->Text(150,7,'RETIRO',0,'C', 0);
  }else{
  $pdf->Text(150,7,'ENTREGA',0,'C', 0);  
  }  
...
//AFORO
  ...
  for($i=0;$i<1;$i++)
        ...
        {
            $pdf->Row(array('CODIGO', 'SERVICIO', 'OBSERV.', 'CANT.', 'PRECIO','I.V.A.','TOTAL'));
        }
      
  $CodigoSeguimiento=$fila['CodigoSeguimiento'];
  $historial = $con->conectar();	
  $strConsulta = "SELECT Codigo,Titulo,Comentario,Precio,Cantidad,ImporteNeto,Iva3,Total FROM Ventas WHERE NumPedido='$CodigoSeguimiento' AND Eliminado=0";
  $historial = mysql_query($strConsulta);
  $numfilas = mysql_num_rows($historial);
  //Calcula el total de la repo
  setlocale(LC_ALL,'es_AR');
  $Muestra=mysql_query("SELECT SUM(Cantidad)as TotalCantidad,SUM(ImporteNeto) as TotalNeto,SUM(Iva3)as TotalIva,SUM(Total)as Total FROM Ventas WHERE NumPedido='$CodigoSeguimiento'");
  $i=0;
  $row=mysql_fetch_array($Muestra);
  $TotalCant=$row[TotalCantidad];
  $Neto=number_format($row[TotalNeto],2,',','.');
  $Iva=number_format($row[TotalIva],2,',','.');
  $Iva_total_label=number_format($row[TotalIva],2,',','.');  
...
for ($i=0; $i<$numfilas; $i++)
      ...
      if($i%2 == 1)
        ...
        $pdf->SetTextColor(0,0,0);//color negro
        $pdf->SetFillColor(255,255,255);  
				$pdf->Row(array($filaVentas['Codigo'], $filaVentas['Titulo'], $filaVentas['Comentario'], $filaVentas['Cantidad'],'$ '.$ImporteNeto_label,'$ '.$Iva_precio_label,'$ '.$Total));
			}
			else
			{
        $pdf->SetFont('Arial','B',5);
  ...
  $pdf->SetFont('Arial','B',8);
	...
	$pdf->SetY(-54);
	$pdf->SetFont('Arial','',6);
  $pdf->Cell(100,6,utf8_decode('La presente Guia de Carga (Carga de Porte) es el único titulo legal del contrato de transporte y su prueba entre todas las partes involucradas');
