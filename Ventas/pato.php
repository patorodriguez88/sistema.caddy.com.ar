...
$BuscarCliente=mysql_query("SELECT * FROM Clientes WHERE idProveedor='$Clientes[$i]' AND Relacion='$_SESSION[idClienteRelacion]';");
...
//  		$Fecha=date('Y-m-d');
    ...
    $Cero='0';
		...
		VALUES('{$FechaCarga}','{$cliente}','{$CuitClienteA}',
		...
		'{$Transportista}','{$Recorrido}','{$ProvinciaDestino}','{$ProvinciaOrigen}','{$FechaEnvio}')";
    ...
    `idCliente`,
    `Posicion`,
    `Celular`)VALUES ('{$FechaEnvio}','{$Recorrido}','{$DomicilioDestino}','{$LocalidadDestino}','{$ProvinciaDestino}','{$Pais}',
...
//  		$Fecha= date("Y-m-d");	
    ...
    $Observaciones='Ya tenemos tu pedido!';  
      ...
      $sqlSeg="INSERT INTO Seguimiento(Fecha,Hora,Usuario,Sucursal,CodigoSeguimiento,Observaciones,Entregado,Estado)
		VALUES('{$FechaEnvio}','{$Hora}','{$Usuario}','{$Sucursal}','{$NumeroPedido}','{$Observaciones}','{$Entregado}','{$Estado}')";
		mysql_query($sqlSeg);
}
