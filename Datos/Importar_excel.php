...
<form class='form-control' action='' method='POST' style='width:500px'>
<label id="pagadestino_label">Cliente Emisor </label>
  <select name="razonsocial" class="form-control select2" data-toggle="select2" >
...
// }
// echo "</select></div>";    
// echo "<div><input type='submit' name='cargar' value='Aceptar' size='40'/></div>";
// echo "</form>";
// goto a;
}
...
skin_open();
echo ('<caption>Subir Archivos de Asignaciones</caption>');
skin_close();
...
{ if (is_uploaded_file($_FILES["dumpfile"]["tmp_name"]) && ($_FILES["dumpfile"]["error"])==0)
  { 
    ...
    { echo ("<p class=\"error\" style='margin-left:20px'>El archivo $uploaded_filename ya existe! Borrelo e intente nuevamente!</p>\n");
    }
    ...
    { echo ("<p class=\"error\" style='margin-left:20px'>Error moving uploaded file ".$_FILES["dumpfile"]["tmp_name"]." to the $uploaded_filepath</p>\n");
      echo ("<p>Verifique los permisos para subir archivos a $upload_dir (must be 777)!</p>\n");
    }
    else
    { echo ("<p class=\"success\" style='margin-left:20px' >Subio con exito el archivo y fue guardado en: $uploaded_filename</p>\n");
    }
  }
  else
  { echo ("<p class=\"error\" style='margin-left:20px'>Error al subir el archivo  ".$_FILES["dumpfile"]["name"]."</p>\n");
  }
}
...
{ if (preg_match("/(\.(sql|gz|csv))$/i",$_REQUEST["delete"]) && @unlink($upload_dir.'/'.$_REQUEST["delete"])) 
    echo ("<p class=\"success\" style='margin-left:20px'>".$_REQUEST["delete"]." fue removido exitosamente</p>\n");
  else
    echo ("<p class=\"error\" style='margin-left:20px'>No se puede borrar el archivo ".$_REQUEST["delete"]."</p>\n");
}
...
{ $dbconnection = @mysql_connect($db_server,$db_username,$db_password);
  ...
  if (!$dbconnection || !$db) 
  { echo ("<p class=\"error\">Fallo la conexion a la base de datos due to ".mysql_error()."</p>\n");
  ...
  { 
    ...
    { 
      ...
      { 
        ...
        { if (!$dirhead)
          { echo ("<table class='login' style='width:97%;margin-left:20px' ");
//             echo ("<caption>Subir Archivos de Stock:</caption>");
           echo ("<tr><th>Cliente Emisor</th><th>Filename</th><th>Size</th><th>Date&amp;Time</th><th>Type</th><th>&nbsp;</th><th>Eliminar</th>\n");
            $dirhead=true;
          }
          echo ("<tr><td style='color:red'>$_SESSION[imp_razonsocial]</td><td>$dirfile</td><td class=\"right\">".filesize($upload_dir.'/'.$dirfile)."</td><td>".date ("Y-m-d H:i:s", filemtime($upload_dir.'/'.$dirfile))."</td>");
          ...
          if ((preg_match("/\.gz$/i",$dirfile) && function_exists("gzopen")) || preg_match("/\.sql$/i",$dirfile) || preg_match("/\.csv$/i",$dirfile))
            echo ("<td><a href=\"".$_SERVER["PHP_SELF"]."?start=1&amp;fn=".urlencode($dirfile)."&amp;foffset=0&amp;totalqueries=0&amp;delimiter=".urlencode($delimiter)."\">Subir Archivo</a> into $db_name at $db_server</td>\n <td><a href=\"".$_SERVER["PHP_SELF"]."?delete=".urlencode($dirfile)."\">Delete file</a></td>
            
            </tr>\n");

//            $dirfile1=explode(".",$dirfile);
//            $dia=substr($dirfile1[0],0,2);
//            $mes=substr($dirfile1[0],2,2);
//            $ano=substr($dirfile1[0],4,2);
//            $Fecha0=$ano.'-'.$mes.'-'.$dia;
//            $NumeroFecha=$dia.''.$mes.''.$ano;
...
// Test permissions on working directory
  ...
  { fclose($tempfile);
    ...
    echo ("<p style='margin-top:20px;margin-left:20px'>Recuerde que no se podran subir archivos mayores a $upload_max_filesize bytes (".round ($upload_max_filesize/1024/1024)." Mbytes)</p>\n");
//     echo ("directamente desde el navegador al server. De ser necesario contactese con el administrador para realizarlo via FTP.</p>\n");

  
?>
<form class="login" method="POST" action="<?php echo ($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data" style="margin-top:20px">
<input type="hidden" name="MAX_FILE_SIZE" value="$upload_max_filesize">
<div><label>Seleccione el archivo:</label><input type="file" name="dumpfile" accept="*/*" size="60"></div>
<div><input type="submit" name="uploadbutton" value="Upload" widht="100px"></div>
</form>
<?php
  }
}
...
// Create an SQL query from CSV line
  ...
  $dumpline = 'INSERT INTO '.$csv_insert_table.' (NCliente,Fecha,Observaciones,idProveedor,ClienteDestino,DomicilioDestino,LocalidadDestino,Provincia,Telefono) VALUES ('.$NCliente.','.$dumpline.');';
...
//DETERMINO LAS VARIABLES FIJAS PARA LA IMPORTACION
  ...
  if($datosqlclientes[id]==null){
    $sql = mysql_query("INSERT INTO Clientes (`NdeCliente`, `nombrecliente`,`Direccion`,`Ciudad`, `Provincia`, `Telefono`,`Celular`,`Observaciones`,
   `Relacion`,`idProveedor`) VALUES (id,'{$row[RazonSocial]}','{$row[DomicilioDestino]}','{$row[LocalidadDestino]}','{$row[ProvinciaDestino]}','{$row[Telefono]}',
   '{$row[Celular]}','{$row[Observaciones]}','{$NCliente}','{$row[idProveedor]}')");
