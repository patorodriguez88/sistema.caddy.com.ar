  ...
  if(in_array($_FILES["file"]["type"],$allowedFileType)){
        ...
        for($i=0;$i<$sheetCount;$i++)
        {
            ...
            foreach ($Reader as $Row)
            {
                ...
                $Fecha = "";
                if(isset($Row[0])) {
                    $Fecha0 = mysqli_real_escape_string($con,$Row[0]);
                    $Fecha1 = explode(" ",$Fecha0);//SEPARO LA FECHA DE LA HORA
                    $Fecha3 = explode("/",$Fecha1[0]);//INVIERTO LA FECHA
                    $Fecha=$Fecha3[2]."-".$Fecha3[1]."-".$Fecha3[0];//AGREGO CARACTER DE SEPARACION
                }
                
                $Patente = "";
                if(isset($Row[13])) {
                    $Patente = mysqli_real_escape_string($con,$Row[13]);
                }
				
                $Producto = "";
                if(isset($Row[16])) {
                    $Producto = mysqli_real_escape_string($con,$Row[16]);
                }
				
                $Litros = "";
                if(isset($Row[17])) {
                    $Litros	= mysqli_real_escape_string($con,$Row[17]);
                }   

                $Odometro = "";
                if(isset($Row[14])) {
                    $Odometro = mysqli_real_escape_string($con,$Row[14]);
                }

                $Tarjeta = "";
                if(isset($Row[8])) {
                    $Tarjeta	= mysqli_real_escape_string($con,$Row[8]);
                }

                $Conductor = "";
                if(isset($Row[11])) {
                    $Conductor	= mysqli_real_escape_string($con,$Row[11]);
                }
                $Remito = "";
                if(isset($Row[15])) {
                    $Remito	= mysqli_real_escape_string($con,$Row[15]);
                }
                $Total = "";
                if(isset($Row[19])) {
                    $Total	= mysqli_real_escape_string($con,$Row[19]);
                }
                 
            //   if($Patente<>'IDENTIFICACION TARJETA'){
                // if (!empty($nombrecliente) || !empty($Direccion)) {
                    $NCliente=1;

                    // $query_control="SELECT id FROM Ypf WHERE Fecha='".$Fecha."' AND Remito='".$Remito."'";
                    // $dato=mysqli_query($con,$query_control);
                    // $dato_control=mysqli_num_rows($dato);
                    
                    // if($dato_control<>0){
                    
                    //     $message = "Hay ".$dato_control." repetidos o iguales";
                    //     $type = "error";
                    
                    // }else{
                    

                            $query ="INSERT INTO Ypf(`Fecha`,`Patente`,`Producto`,`Litros`,`Odometro`,`Tarjeta`,`Conductor`,`Total`,`Remito`)  
                                    VALUES('".$Fecha."','".$Patente."','".$Producto."','".$Litros."','".$Odometro."','".$Tarjeta."','".$Conductor."','".$Total."','".$Remito."')";                 
