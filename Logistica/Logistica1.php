...
$Orden=$_GET['orden_t'];	
$sql=mysql_query("SELECT * FROM Logistica WHERE NumerodeOrden='$Orden' AND Eliminado=0");
...
if ($_GET['Orden']=='Cargar'){
	$idKilometros=mysql_query("SELECT * FROM Vehiculos WHERE Dominio='$Patente'");
	$fila1=mysql_fetch_array($idKilometros);
  $sqlcontrolo=mysql_query("SELECT id FROM Logistica WHERE Patente='$Patente' AND (Estado='Alta' OR Estado='Cargada' OR Estado='Pendiente')");
...
// INCLUIR LOS QUE DICEN DEJAR FIJO	
  // PRIMERO VERIFICO QUE EL RECORRIDO ASIGNADO TENGA CLIENTE EMISOR POR MEDIO DEL SERVICIO
  $sqlrecorrido=mysql_query("SELECT Cliente FROM Recorrido WHERE Numero='$Recorrido'");
...
// $sql4="UPDATE HojaDeRuta SET NumerodeOrden ='$Numero',Posicion='$BP1' WHERE Recorrido='$Recorrido' AND Estado='Abierto'";
// mysql_query($sql4);
$sqlfechaorden=mysql_query("SELECT Fecha FROM Logistica WHERE NumerodeOrden='$Orden' AND Eliminado=0");
$datofechaorden=mysql_fetch_array($sqlfechaorden);
  
$sql5="UPDATE TransClientes SET Transportista ='$Chofer', FechaEntrega='$datofechaorden[Fecha]' WHERE Recorrido='$Recorrido' AND Entregado='0' and Eliminado='0'";
mysql_query($sql5);
