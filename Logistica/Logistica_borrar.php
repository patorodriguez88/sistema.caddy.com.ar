...
//-----------------------------------------------DESDE ACA VER ORDENES---------------------------
	...
	if($_GET['Recorrido']==''){
      ...
      GROUP BY Recorrido ORDER BY Recorrido ASC";
			...
			while ($row = $estructura->fetch_row(MYSQLI_ASSOC)){
			echo "<option value='".$row[Recorrido]."'>".$row[0].' '.$row[1]."</option>";
			}
			echo "</select></div>";
		echo "</form>";	
		goto a;
...
$Estado='Alta';
  ...
  <?  
    ...
    if($Recorrido==""){
        ...
        $resultado=$mysqli->query($SQL_NUEVO);
	...
	$sql="INSERT IGNORE INTO Logistica(
			NumerodeOrden,
			Fecha,
			Hora,
			Controla,
...
// echo "<p><button data-user='1'>Usar mis datos</button></p>";
 echo "<div><label>Numero de orden 740:</label><input name='orden_t' type='text' value='".$file[NumerodeOrden]."' style='width:120px;'/></div>";
...
if ($_GET['Orden']=='Cargar'){
  ...
  $IdUsuarioChofer=$fila[Usuario];
	...
	$sql="UPDATE IGNORE Logistica SET 
    Patente='$Patente',
    ...
    if(!$result){
        echo "Error en inserción ".$mysqli->error;
    }


$sql2="UPDATE IGNORE Vehiculos SET Estado ='Alta en recorrido $Recorrido',Observaciones ='$Observaciones' WHERE Dominio='$Patente'";
...
$sql3="UPDATE IGNORE HojaDeRuta SET NumerodeOrden ='$Orden' WHERE Recorrido='$Recorrido' AND Estado='Abierto' AND Eliminado='0' AND Devuelto='0'";
...
$sql5="UPDATE IGNORE TransClientes SET Transportista ='$Chofer', FechaEntrega='$datofechaorden[Fecha]',NumerodeOrden='$Orden' WHERE Recorrido='$Recorrido' AND Entregado='0' AND Eliminado='0' AND Devuelto='0' AND Haber='0'";
...
//SERVICIO
  ...
  //SI EL RECORRIDO TIENE CARGADO EL SERVICIO CARGO EN LA CUENTA CORRIENTE DEL CLIENTE EL IMPORTE
    $FechaHoy= date("Y-m-d");	  
    $sqlctacte=$mysqli->query("INSERT IGNORE INTO `Ctasctes`(`Fecha`,`RazonSocial`,`Cuit`,`TipoDeComprobante`,`NumeroVenta`,`Debe`, `Usuario`,`idCliente`,`Observaciones`,`FacturacionxRecorrido`,idLogistica) VALUES 
...
$sqlEstado="UPDATE IGNORE TransClientes SET Estado='$Estado' WHERE id='$row[id]'";
...
$sql="INSERT IGNORE INTO Seguimiento(Fecha,Hora,Usuario,Sucursal,CodigoSeguimiento,Observaciones,Entregado,Estado,Destino,Recorrido,NumerodeOrden)
...
$result=$mysqli->query("UPDATE IGNORE HojaDeRuta SET Estado='Abierto',Recorrido='$Recorrido',NumeroDeOrden ='$Orden' WHERE Eliminado='0' AND Devuelto='0' AND Seguimiento='$CodigoSeguimiento' LIMIT 1");  
if(!$result){
    echo "Error en inserción ".$mysqli->error;
}
}
...
$IngresaRoadmap=$mysqli->query("INSERT IGNORE INTO `Roadmap`(`Fecha`,`Recorrido`, `Localizacion`, `Ciudad`,
...
//------------------------------------------------HASTA ACA CARGAR ORDENES GENERDAS-----------------------------------------------
	if ($_GET['id']=='Cerrar'){
        ...
        echo "<div><hr></hr></div>";  
        echo "<div><label>Numero de orden 1295:</label><input name='orden_t' type='text' style='width:100px;'></div>";
...
$KmRecorrido_row=$sqlkm->fetch_array(MYSQLI_ASSOC);
$KmRecorrido=$KmRecorrido_row['Kilometros'];
...
echo "<div><label>Numero de orden 1325:</label><input name='orden_t' type='text' value='$Orden' style='width:150px;'/></div>";
echo "<div><label>Controla:</label><input name='controla_t' type='text' value='".$_SESSION['Usuario']."' style='width:150px;'/></div>";
...
echo"<div><label>Cargo combustible:</label><input type='text' value='0' name='carga_t' style='width:50px;' placeholder='litros' ></div>";
